import React from 'react';
import { Image as AntdImage } from 'antd';
import { ImageProps as AntdImageProps } from 'antd/lib/image';
import { LazyLoadImage } from 'react-lazy-load-image-component';


// type PropsType = {
//   src: string;
//   height: string | number;
//   position?: boolean;
//   width?: string | number;
//   left?:  string | number | undefined
//   top?:  string | number | undefined
// }
//
// export const MyImage: React.FC<PropsType> = (props) => {
//   const { src, height, position = false, width = '100%', left = "0",  top = '20' } = props;
//   console.log(props);
//   const fallback = "data:image/png;base64,"
//   // props.position ? return <div style={{position: "absolute", top: 20}}><Image preview={false} src={props.src} height={props.height} width={130}  fallback={fallback}></Image></div> : return <div style={{position: "absolute", top: 20}}><Image preview={false} src={props.src} height={props.height} width={130}  fallback={fallback}></Image></div>
//   if (position) {
//     return <div style={{position: "absolute", top: top, left:left}}><Image preview={false} src={src} height={height} width={width}  fallback={fallback}></Image></div>
//   } else {
//     return  <div><Image preview={false} src={props.src} height={props.height} width={width}  fallback={fallback}></Image></div>
//   }
// }

interface ImageProps extends AntdImageProps {
  src?: string;
  alt?: string;
  type?: 'lazy' | '';
}

export const Image: React.FC<ImageProps> = ({ src, alt, type, ...restProps }) => {
  // const fallbackSrc = 'https://via.placeholder.com/150x150.png?text=Image+not+found';
  const fallbackSrc = 'data:image/png;base64,';
  //
  // if (type === 'lazy') {
  //   return <LazyLoadImage
  //     alt={alt}
  //     effect="opacity"
  //     height={180}
  //     width={180}
  //     src={src} // use normal <img> attributes as props
  //   />;
  // }

  const imageLoading = () => {
    PubSub.publish("imageLoading")
  }

  return (
    <AntdImage
      {...restProps}
      src={src}
      fallback={fallbackSrc}
      preview={false}
      loading="lazy"
      alt={alt || 'Image'}
      onLoad={imageLoading}
    />
  );
};
